...
///
/// - Note: There is a default implementation, `DefaultDefaultsKey`, that Pippin uses in case you don't provide your own `Defaults` when initializing a new `Environment`. Check its implementation for reference when creating your own, both for its usage of an `enum` and its usage of `String(asRDNSForPippinSubpaths:)` to create a likely-unique key for the backing store.
